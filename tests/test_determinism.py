...
from .conftest import make_conditions, make_pier
...
def test_two_runs_are_byte_identical_offline() -> None:
    ...
    with no_network():
        first = produce_verdict(pier, clock, conditions).to_json()
        second = produce_verdict(pier, clock, conditions).to_json()

    assert first == second

    payload = json.loads(first)
    # The identity spans real astronomy and a real conditions-driven verdict,
    # not the M1 null stubs.
    ...
    # A real, gate-passing verdict from the pinned snapshot: a non-null integer
    # score and at least one itemised reason.
    assert payload["verdict"] in {"GO", "MAYBE"}
...
def test_production_makes_no_network_access() -> None:
    ...
    with no_network():
        doc = produce_verdict(make_pier(), FixedClock(_PINNED_INSTANT), make_conditions())
