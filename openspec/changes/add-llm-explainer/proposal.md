...
- Add an **explainer provider interface** with a default Anthropic (Claude)
  provider, mirroring the conditions-provider edge seam. The provider is
  **disabled by default** and enabled through configuration (model + API key).
...
- **New code:** an explainer edge package (e.g. `pierpressure/explain/`) holding
  the provider interface, the default Anthropic provider, the input-building and
  ...
  from `core/`, making the new boundary mechanically enforced rather than merely
  currently true.
...
- **Dependencies:** a new runtime dependency on the Anthropic SDK (added with
  `uv add`), used only inside the explainer edge.
